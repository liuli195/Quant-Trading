...
源 Issue：https://github.com/liuli195/Quant-Trading/issues/44
...
- Claude Code 通过 `.claude/skills` Directory Symlink 指向 `.agents/skills`，读取同一份内容；Symlink 由 Git 通过 `core.symlinks=true` 直接追踪。
...
修改一个 Skill 只需要改 `.agents/skills/<skill>/` 一处。治理检查从 adapter 等价性改为 `.agents/skills` 完整性和 `.claude/skills` Symlink 有效性检查（同时检查 `CLAUDE.md` Symlink 有效性）。
