...
以下函数定义在 [../snippets/extract.js](../snippets/extract.js) <!-- pathref: joinquant_skill/snippets/extract.js -->。流程文档只引用路径名称，具体调用约定以本节为准。
...
DOM 降级受虚拟滚动限制，`transactioninfo`、`positioninfo`、`logs` 可能只是当前可见部分，必须按 [output-contract.md](output-contract.md) <!-- pathref: joinquant_skill/reference/output-contract.md --> 标记完整度。
