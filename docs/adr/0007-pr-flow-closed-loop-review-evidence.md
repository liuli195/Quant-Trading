...
- Standards / Spec fragments 必须记录 `delegation_attempt`，区分已派发的 `spawned` review 与 `tool_unavailable` / `spawn_failed` fallback review。授权缺失类理由 `user_not_authorized`、`permission_not_allowed`、`explicit_authorization_missing`、`policy_disallowed` 不能作为 fallback reason。
