...
- `pr-complete` 语义、远端 merge policy 和 cleanup 收尾仍有人工切换成本。

本 ADR 对应 PRD Issue：<https://github.com/liuli195/Quant-Trading/issues/27>。

## 决策

- `pr-ready` 只推进到 merge-ready，永不合并；`pr-complete` 表示用户授权在全部规则满足后自动 ready-for-review、head-locked merge 和 cleanup。
- `.local/pr-flow/state.json` 和 `.local/pr-flow/last-status.json` 只作为本地缓存和诊断面；权威证据只来自当前 Git 状态、GitHub 状态和校验过的 review evidence。
...
- cleanup 必须在确认 PR merged、本地 base 分支受控 fast-forward 且 `main...origin/main = 0 0` 后，才删除本地和远端已合并 head branch，并输出最终摘要。
