...
- missing / stale local review fragments 生成 `.local/pr-flow/review-fragments-handoff.json`；agent-only builder 只接受结构化 reviewer verdict payload，允许 same-diff head refresh，diff changed 必须重新 review，失败不写半成品 fragment。
