...
审计覆盖工具登记、README/文档/测试锚点、CLI help、workflow template schema、`CLAUDE.md`、`jq-research`/`jq-ab-test` Skill、数据 catalog、报告 catalog 和 pathref。`gate` 是本地 hook 和 CI 使用的门禁入口，会同时运行治理审计和 pathref 校验。
...
登记项包含 `tool_id`、`library`、`layer`、`kind`、`entry_module`、`cli`、README、文档、测试锚点、输入和输出。CLI 元数据必须使用 `.\.venv\Scripts\python.exe -m ...`。
