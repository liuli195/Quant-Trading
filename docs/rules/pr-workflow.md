...
- Standards / Spec fragment 必须记录 `delegation_attempt`：`required=true`、`authorization_basis="AGENTS.md + ADR 0009"`、`tool="spawn_agent"`、`result=spawned|tool_unavailable|spawn_failed`。`spawn_failed` 是失败态，不能满足本地交叉评审证据；`empty_completion` 只是 `spawn_failed` 的诊断 reason，必须诊断并重试或人工介入。`tool_unavailable` 只表示工具未注册、不可用或不可调用，不能承载 `empty_completion`、timeout、cancelled、tool error、malformed response；授权缺失类理由仍禁止使用。
