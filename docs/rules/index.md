...
MUST 规则变更必须同步规则文档、依赖脚本、测试、索引；影响协作模型、目录结构、门禁或策略流程时，同步 [docs/adr](../adr) <!-- pathref: docs/adr --> 并经过 CODEOWNER review。任何与规则冲突的改动、对规则本身的改动都必须显式获得授权（见 AGENTS.md 核心规则「规则优先」）。
