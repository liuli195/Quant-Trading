...
- Codex Cloud/Linux 使用 `.githooks/run-python.sh`，由 wrapper 设置 UTF-8 并调用 `.venv/bin/python`。
- 裸用 `.\.venv\Scripts\python.exe -m ...` 仍可用于临时排障；若命令输出中文，需先设置 `PYTHONUTF8=1` 和 `PYTHONIOENCODING=utf-8`。
