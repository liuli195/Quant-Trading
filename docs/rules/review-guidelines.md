...
- Standards / Spec fragment 必须记录 `delegation_attempt`，证明是否已调用 `spawn_agent`。允许结果只有 `spawned`、`tool_unavailable`、`spawn_failed`；`spawn_failed` 是失败态，不能生成通过 fragment，必须诊断并重试或人工介入；`tool_unavailable` 只表示工具未注册、不可用或不可调用，不能承载 `empty_completion`、timeout、cancelled、tool error、malformed response 这类已派发失败。禁止使用 `user_not_authorized`、`permission_not_allowed`、`explicit_authorization_missing`、`policy_disallowed` 这类授权缺失理由。
