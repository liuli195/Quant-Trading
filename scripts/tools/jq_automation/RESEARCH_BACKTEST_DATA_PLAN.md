...
```powershell
.\.venv\Scripts\python.exe -m scripts.tools.jq_automation fetch <backtestId> --strategy <strategy> --result-source auto
.\.venv\Scripts\python.exe -m scripts.tools.jq_automation run <scenario.json> --yes --result-source research
.\.venv\Scripts\python.exe -m scripts.tools.jq_automation batch <manifest.json> --yes --result-source detail
.\.venv\Scripts\python.exe -m scripts.tools.jq_automation ab run <ab_config.json> --yes --result-source auto
```
...
- JoinQuant research 页存在 `/hub/user/<id>/...` 到 `/user/<id>/...` 的中转跳转；前者是 Hub bridge，不是可执行工作区。执行器必须等待进入正式 `/user/` 工作区后再探测和创建 kernel，否则会命中看似可读、实际拒绝写入的 405 响应。
