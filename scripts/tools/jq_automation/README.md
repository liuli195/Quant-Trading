...
```text
签名: python -m scripts.tools.jq_automation compile-check <strategy_file> [--write-upload]
...
```text
签名: python -m scripts.tools.jq_automation upload <strategy_file> [flags]
...
```text
签名: python -m scripts.tools.jq_automation run <scenario.json> [flags]
...
```text
签名: python -m scripts.tools.jq_automation fetch <backtestId|detail-url> [flags]
...
```text
签名: python -m scripts.tools.jq_automation batch <manifest.json> [flags]
...
```text
签名: python -m scripts.tools.jq_automation ab expand <ab_config.json> [--force-reset-pending]
...
```text
签名: python -m scripts.tools.jq_automation ab run <ab_config.json> [flags]
...
```text
签名: python -m scripts.tools.jq_automation ab report <config_or_manifest> --experiment <id> [--allow-partial]
...
工具落盘通过 `path_aliases.json` 中的 alias 解析，不硬编码路径。脚本中新增结果目录引用应使用 `scripts.tools.path_tools.aliases.resolve_path()` 或 `ensure_dir()`。
...
scripts/tools/jq_automation/
  __main__.py              # python -m scripts.tools.jq_automation 入口
