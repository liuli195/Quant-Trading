...
async def _research_context(page: Any) -> Any:
    ...
    if _is_direct_user_workspace_url(str(getattr(page, "url", ""))):
        return page
    ...
    for _ in range(60):
        ...
        if frame is not None:
            ...
            if _is_direct_user_workspace_url(last_url):
                try:
...
def _is_direct_user_workspace_url(url: str) -> bool:
    """Return True only for the real `/user/<id>/...` workspace, not the Hub bridge."""
    path = urlparse(str(url or "")).path
    return path.startswith("/user/") and not path.startswith("/hub/user/")
...
async ({ code }) => {
  ...
  async function findApiBases(candidates) {
    const found = [];
    const errors = [];
  ...
  let kernelId = "";
  try {
    const discoveredBases = discoverBases();
    log.push(`locationHref=${location.href}`);
    log.push(`discoveredBases=${discoveredBases.join(",")}`);
    const apiBases = await findApiBases(discoveredBases);
    log.push(`apiBases=${apiBases.join(",")}`);
    let apiBase = "";
    let kernel = null;
