...
```python
from scripts.tools.path_tools.aliases import resolve, resolve_path
```
...
```bash
python -m scripts.tools.path_tools.refactor rewrite-md --dry-run
python -m scripts.tools.path_tools.refactor rewrite-md
```

Move a file or directory and update references:

```bash
python -m scripts.tools.path_tools.refactor move old/path.md new/path.md
```

Apply multiple moves:

```bash
python -m scripts.tools.path_tools.refactor rewrite --map moves.json --dry-run
python -m scripts.tools.path_tools.refactor rewrite --map moves.json
```
