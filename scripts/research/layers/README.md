...
生成命令：
- Windows：`powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\.githooks\run-python.ps1 -m scripts.research.registry.tool_registry write-layers`
- POSIX：`.githooks/run-python.sh -m scripts.research.registry.tool_registry write-layers`
...
| 横向治理：注册与审计 | [governance.md](governance.md) <!-- pathref: scripts/research/layers/governance.md --> | 跨层注册、审计、路径引用和提交门禁。 | 4 |
