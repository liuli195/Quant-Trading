...
为 `scripts/jq_automation` 增加声明式 A/B 测试能力：用户通过一个实验配置声明 baseline、多个 controls、variants、参数差异和 Git 策略版本；工具把每个候选项生成一份最终上传的完整策略 `.py` 快照，在一次聚宽策略编辑器上传会话中顺序覆盖上传、编译、回测、抓取结果，并生成 A/B 对比报告。
...
```powershell
python -m scripts.jq_automation ab expand <ab_config> [--force-reset-pending]
python -m scripts.jq_automation ab run <ab_config> --yes [--backtest-timeout 180]
python -m scripts.jq_automation ab report <ab_config-or-manifest> --experiment <id> [--allow-partial]
```
...
新增 `scripts/jq_automation/abtest.py`：
...
新增 `scripts/jq_automation/git_versioning.py`：
...
新增 `scripts/jq_automation/metrics.py`：
