...
本文档只描述执行编排与决策点。DOM 选择器、等待条件、内部数据 API、提取函数和 bundle 字段契约统一见 [browser-contracts.md](browser-contracts.md) <!-- pathref: agents_joinquant_skill/reference/browser-contracts.md -->；产物结构见 [output-contract.md](output-contract.md) <!-- pathref: agents_joinquant_skill/reference/output-contract.md -->。
...
- 读取 [../snippets/editor.js](../snippets/editor.js) <!-- pathref: agents_joinquant_skill/snippets/editor.js -->。
...
- 读取 [../snippets/compile.js](../snippets/compile.js) <!-- pathref: agents_joinquant_skill/snippets/compile.js -->。
- 等待与失败判定按 [browser-contracts.md](browser-contracts.md) <!-- pathref: agents_joinquant_skill/reference/browser-contracts.md --> 的“编译完成”规则执行。
- 若失败，打开错误日志、提取证据、修复本地策略后从第 3 步重新执行；证据保留要求见 [troubleshooting.md](troubleshooting.md) <!-- pathref: agents_joinquant_skill/reference/troubleshooting.md -->。

## 7. 设置并启动正式回测

- 读取 [../snippets/backtest.js](../snippets/backtest.js) <!-- pathref: agents_joinquant_skill/snippets/backtest.js -->。
- 设置开始日期、结束日期和初始资金。
- 日期归一化规则：
...
- 按 [browser-contracts.md](browser-contracts.md) <!-- pathref: agents_joinquant_skill/reference/browser-contracts.md --> 的“正式回测完成”规则等待。
- 若超时，按 [troubleshooting.md](troubleshooting.md) <!-- pathref: agents_joinquant_skill/reference/troubleshooting.md --> 的回测超时流程处理。

## 9. 提取数据

- 读取 [../snippets/extract.js](../snippets/extract.js) <!-- pathref: agents_joinquant_skill/snippets/extract.js -->。
- 已有详情页抓取模式：按 [browser-contracts.md](browser-contracts.md) <!-- pathref: agents_joinquant_skill/reference/browser-contracts.md --> 的“API bundle 主路径”执行，只读抓取详情页数据，禁止点击会消耗积分的页面“导出”。
- 上传并新跑回测模式：优先按 [browser-contracts.md](browser-contracts.md) <!-- pathref: agents_joinquant_skill/reference/browser-contracts.md --> 的“新跑回测 API 路径”提取；不可用时按“DOM 降级路径”提取。
- 保存浏览器返回的原始 JSON：
...
路径、文件职责和完整度标记以 [output-contract.md](output-contract.md) <!-- pathref: agents_joinquant_skill/reference/output-contract.md --> 为准。需要绝对路径时，先解析并记录本次别名：
