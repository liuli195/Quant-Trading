...
原技术方案：[ETF轮动策略技术实现方案.md](ETF轮动策略技术实现方案.md) <!-- pathref: strategy_reports(strategy=etf_factor_rotation)/design/ETF轮动策略技术实现方案.md -->  
原方案说明：[ETF轮动策略方案说明书.md](ETF轮动策略方案说明书.md) <!-- pathref: strategy_reports(strategy=etf_factor_rotation)/design/ETF轮动策略方案说明书.md -->  
...
| `MomentumExtremeScoreStart` | `None` | 极端高动量弱化阈值；`None` 表示关闭 | 更早把高分资产压回中性 |
| `MomentumExtremeTiltCap` | `1.00` | 命中极端高动量阈值后的倾斜上限 | 允许保留更多高位加仓 |
...
MomentumExtremeScoreStart is None or 0 < MomentumExtremeScoreStart <= 1
1 <= MomentumExtremeTiltCap <= MomentumTiltMax
...
if MomentumExtremeScoreStart is not None and MomentumScore_i >= MomentumExtremeScoreStart:
    MomentumTilt_i = min(MomentumTilt_i, MomentumExtremeTiltCap)
```
...
- 默认关闭 `MomentumExtremeScoreStart`，保持历史线性倾斜行为；启用后只弱化极端高分，不改变中段增强。
...
if params["MomentumExtremeScoreStart"] is not None and not (0 < params["MomentumExtremeScoreStart"] <= 1):
...
- `snapshot_params()` 包含 7 个新增倾斜参数。
- 默认参数通过 `validate_params()`。
- `MomentumTiltStrength < 0` 抛出错误。
- `MomentumTiltMin <= 0`、`MomentumTiltMin > 1`、`MomentumTiltMax < 1` 抛出错误。
- `MomentumExtremeScoreStart <= 0`、`MomentumExtremeScoreStart > 1` 抛出错误。
- `MomentumExtremeTiltCap < 1`、`MomentumExtremeTiltCap > MomentumTiltMax` 抛出错误。
...
- 默认关闭极端高动量弱化时，输出保持原线性映射。
- 达到极端高分阈值后，原本超过 cap 的倾斜被压回 cap。
- 命中阈值但原倾斜低于 cap 时，输出保持不变。
- 单活跃资产即使命中阈值，也仍保持中性倾斜。
...
- [ETF轮动策略方案说明书.md](ETF轮动策略方案说明书.md) <!-- pathref: strategy_reports(strategy=etf_factor_rotation)/design/ETF轮动策略方案说明书.md -->
- [ETF轮动策略技术实现方案.md](ETF轮动策略技术实现方案.md) <!-- pathref: strategy_reports(strategy=etf_factor_rotation)/design/ETF轮动策略技术实现方案.md -->
- [测试方案设计文档.md](../testing/测试方案设计文档.md) <!-- pathref: strategy_reports(strategy=etf_factor_rotation)/testing/测试方案设计文档.md -->
